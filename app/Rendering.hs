...
render g = pictures $ renderWorld g ++ [renderPlayer g] ++ renderGhosts g
...
renderPlayerType (PacMan position score direction) = translatePictureByBoundary position $ getPacManTexture direction
renderPlayerType (Ghost position color state ) = translatePictureByBoundary position $ getGhostTexture color state
...
tileToPicture (NotWalkable wallType (x, y))
  ...
  | wallType == DOOR = translatePicture (x, y) door
